...
const FormGenerator = <T extends FieldValues>({
  ...
  const renderErrorMessage = () => (
    <ErrorMessage
      ...
      render={({ message }) =>
        message !== "Required" && (
          <div className="motion-safe:animate-fadeIn mt-1 flex items-center gap-1.5 text-xs text-destructive">
  ...
  const renderInput = () => (
    <Label className="flex flex-col gap-1.5" htmlFor={`input-${id}`}>
      {label}
  ...
  const renderSelect = () => {
    ...
    return (
      <Label htmlFor={`select-${id}`} className="flex flex-col gap-1.5">
        {label}
        <div className="flex flex-col gap-1">
          <Select
            onValueChange={(value) => {
        ...
        <div className="flex flex-col gap-1">
          <div
            className={cn(
              ...
              hasError &&
                "border-destructive focus-within:outline-destructive/30",
